...
def suggest_silver_data_model(entities: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for ent in entities:
        ...
        scd_type = "Type 2" if any(
            a["name"].lower().endswith("_status") or
            a["name"].lower().endswith("_tier") or
            a["name"].lower().endswith("_category") or
            "address" in a["name"].lower()
            for a in attrs
        ) else "Type 1"
    ...
    for i, ent1 in enumerate(entities):
        for ent2 in entities[i+1:]:
            fk_in_ent1 = [
                a["name"] for a in ent1.get("attributes", [])
                if ent2["name"].lower() in a["name"].lower()
            ]
            
            fk_in_ent2 = [
                a["name"] for a in ent2.get("attributes", [])
                if ent1["name"].lower() in a["name"].lower()
            ]
            ...
            if fk_in_ent1 and fk_in_ent2:
                ...
                suggestions["bridge_tables"].append({
                    ...
                    "foreign_keys": [
                        f"{ent1['name'].lower()}_sk",
                        f"{ent2['name'].lower()}_sk"
                    ],
