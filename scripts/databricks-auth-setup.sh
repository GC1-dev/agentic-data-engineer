#!/bin/bash
# Script to set up Databricks authentication and configuration
# Usage: source scripts/databricks-auth-setup.sh [profile] [warehouse_id]
# Example: source scripts/databricks-auth-setup.sh skyscanner-dev 1204fc84c047ff08
...
echo "export DATABRICKS_HOST=\"${DATABRICKS_HOST}\"" >> ~/.bashrc
echo "export DATABRICKS_CONFIG_PROFILE=\"${DATABRICKS_CONFIG_PROFILE}\"" >> ~/.bashrc
echo "export DATABRICKS_WAREHOUSE_ID=\"${DATABRICKS_WAREHOUSE_ID}\"" >> ~/.bashrc
